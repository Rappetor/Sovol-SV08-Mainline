...
> [!NOTE]
> The standard Klipper firmware works on both the toolhead MCU and the mainboard MCU. Originally Sovol made multiple changes to the `stm32f1.c` source for the firmware but they are not needed mandatory. Only the printer now starts up silently; no fans, light and no display during boot.
...
![Klipper makemenu config settings](/images/klipper-firmware-settings-katapult.jpg)<br><br>
<sub>(because we are using Katapult as bootloader, make sure you set the 8 KiB bootloader offset)</sub><br>

3. Press Q to quit and save changes.<br>

4. Run the command to create the Klipper firmware (`klipper.bin`)<br>
`make clean`<br>
`make` <br>
